...
发布规模训练（8192 环境、5000 迭代、RTX 5060 Ti，约 5 小时）配合课程：

- 成功课程在约第 1000 迭代把容差收紧到目标 0.2 rad，之后策略在**满难度**下持续提升（训练末每回合约 6.7 个目标，抓握稳定，`cage_drop` ≈ 0.2）。
- 100 回合确定性 eval（`genelab eval`，tight 0.2 阈值）：**成功率 0.99**（把 cube 重定向到至少一个"稳定保持"的 SO(3) 目标的回合比例），平均回报约 1060，平均回合长度约 591 步。

!!! note "课程是关键"
    没有成功课程时，对齐 mjlab 的奖励权重（重 torque / action-rate / cage 惩罚）会过度正则化成"稳握不转"，短预算下成功率为 **0%**；加上课程后达到 release 级的 0.99。不带课程、用较轻的 Genesis 调权重的短训练约为 0.33。
