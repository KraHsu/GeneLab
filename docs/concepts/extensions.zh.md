...
GeneLab 核心**不**自带机器人、环境、任务，所有内容由下游扩展包提供。CLI 通过三条路径发现
扩展，按以下优先级。
...
这条路径正是 `genelab project new` 默认接好的。
...
- 需要完全显式、可复现的加载顺序（搭配 `--no-entry-points`）。
- 调试尚未安装的包，临时把源码目录塞进 `sys.path`（CLI 也会把当前工作目录加入 `sys.path`）。
...
测试与嵌入脚本使用。同样的幂等守卫生效，即使 CLI 已经自动发现过同一模块，再调用一次也是
no-op。
...
这是最可复现的组合 —— 只加载显式命名的模块。

## 参考扩展

`examples/genelab_examples/` 是参考形态：`pyproject.toml` 带 entry point、顶层有
`register()` 可调用、并提供 `config.py` / `robots.py` / `envs.py` / `tasks.py`。
`tests/fake_extension.py` 是同样形态但削减到最小，供测试套件使用。
