...
`genelab project new` scaffolds a downstream extension package — a self-contained Python project
that registers robots, environments, and tasks into GeneLab's global registries.
...
Once installed (`uv pip install -e ./my_robot_project`), GeneLab discovers the extension on next
invocation without any `--import` flag.

## After scaffolding

```bash
cd my_robot_project
uv pip install -e .          # install your extension into the GeneLab venv
...
- [Extensions](../concepts/extensions.md) — the three extension-loading pathways and how to keep
  loading idempotent.
- [Configs](../concepts/configs.md) — how to type the `TaskCfg.env` field in your `config.py`.
