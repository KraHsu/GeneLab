...
from genelab_soft_terrain.ppo_cfg import (
    g1_mattress_velocity_ppo_runner_cfg,
    go1_soft_velocity_ppo_runner_cfg,
)
...
MATTRESS_VELOCITY_ENV_NAME = "g1-mattress-velocity-env"
MATTRESS_VELOCITY_TASK_ID = "Genelab-Velocity-Mattress-Unitree-G1-v0"
GO1_ROBOT_NAME = "go1"
G1_ROBOT_NAME = "g1"
...
class G1MattressVelocityTask:
    """Trainable G1 velocity-tracking on the pneumatic air-mattress terrain."""

    def __init__(self) -> None:
        self.cfg = TaskCfg(
            name=MATTRESS_VELOCITY_TASK_ID,
            env_name=MATTRESS_VELOCITY_ENV_NAME,
            robot_name=G1_ROBOT_NAME,
            env=g1_mattress_velocity_env_cfg(play=False),
            play_env=g1_mattress_velocity_env_cfg(play=True),
            agent=g1_mattress_velocity_ppo_runner_cfg(),
            trainable=True,
        )
...
def register() -> None:
    ...
    if MATTRESS_VELOCITY_TASK_ID not in TASKS:
        register_task(
            MATTRESS_VELOCITY_TASK_ID,
            G1MattressVelocityTask,
            description="PPO velocity tracking for the Unitree G1 humanoid on a pneumatic "
            "air mattress (one sealed chamber couples the two feet).",
            cfg_type=TaskCfg,
            examples=[
                f"genelab train {MATTRESS_VELOCITY_TASK_ID} --num_envs 4096",
                f"genelab play {MATTRESS_VELOCITY_TASK_ID} --checkpoint PATH/model.pt",
            ],
        )
