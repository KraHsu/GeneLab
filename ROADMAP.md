...
>
> Last reviewed: 2026-05-21 · against `dev` @ `d7a702e`.

### 9.0 Status / 当前状态

**Phase:** **R0 complete** (R0.1 + R0.2 + R0.3 merged) · R1–R7 not started.
...
| Phase R1 — break rl.runner ↔ rl.backends cycle | ⬜ **next recommended** | gated on ADR-0001 acceptance |
...
**Changes to the dependency graph.** `mcp__codebase-memory-mcp__index_status` reports **3,861 nodes / 11,365 edges** post-R0 (was 3,829 / 11,427 at the start of the planning round). Net deltas: +32 nodes (the two new test files plus their parametrized cases) and −62 edges (the indexer re-resolved some edges after R0.3's `exclude_type_checking_imports = true` filter changed how it accounts for type-only references — graph quality, not loss). The hotspots and duplications flagged in the assessment are still present.

**Risks identified (aggregate view).** Detailed per-ADR; the highest-attention items:

- **R1 (ADR-0001)** — quiescent `rl.runner ↔ rl.backends` cycle held together by lazy `importlib`. Any contributor "cleaning up" backend imports would break the package; R1 fixes this permanently. The R0.3 `lint-imports` baseline currently reports 3 violations from this cycle.
...
- **R7 (ADR-0009)** — flipping importlinter from lint-only to blocking is gated on the R0.3 baseline being clean. As of R0.3 merge: 24 distinct cross-layer imports across 3 broken contracts (see below).

**Risks discovered during R0 (new, not in the original list).**
...
- **The layering spec in `target-architecture.md` / ADR-0009 has more drift from current code than the assessment suggested** (R0.3). With `exclude_type_checking_imports = true`, the R0.3 baseline still shows **24 distinct cross-layer imports** across 3 broken contracts. Root causes by category:
  - 3 imports: `rl.backends.{rsl_rl,skrl,sb3} → rl.runner` — addressed by **R1 / ADR-0001**.
...
- **R7 (importlinter blocking flip) is gated on this 24-violation list being cleaned up or explicitly waived.** Each R1–R6 PR should record which baseline violations it eliminates.
...
3. Land **PR R1.1** — break the `rl.runner ↔ rl.backends` cycle (see below). Highest-risk-reduction slice in the remaining R-chain.
4. Fan-out: R1 / R2 / R5 / R6 can run in parallel; R3 → R4 serial.

**Suggested next slice: R1 — Break `rl.runner ↔ rl.backends` cycle (ADR-0001).**

- **Scope.** Extract 9 helpers (`build_bridges`, `build_env`, `close_bridges`, `make_random_policy`, `make_zero_policy`, `resolve_env_cfg`, `resolve_log_dir`, `run_play_loop`, `save_run_params`) from `src/genelab/rl/runner.py` (currently 408 LoC) into a new `src/genelab/rl/_helpers.py`. Each backend (`rl/backends/{rsl_rl,skrl,sb3}.py`) flips its `from genelab.rl.runner import …` to `from genelab.rl._helpers import …`. `rl/runner.py` keeps `train_task` / `play_task` plus a re-export shim for one release (CHANGELOG announces the deprecation). One new test `tests/test_no_static_cycle.py` asserts the cycle stays broken under future changes (imports `genelab.rl.backends.sb3` in a subprocess, asserts `genelab.rl.runner` is *not* in `sys.modules` afterwards).
- **Why next.** (1) **Highest existing-risk reduction.** The cycle is quiescent today only because of the undocumented `_ensure_loaded` lazy-import workaround. Any contributor "cleaning up" backend imports would break the package on import. (2) **Immediate R0.3 baseline cleanup.** Contract 2 (`rl.backends does not import rl.runner`) flips from broken to kept, removing 3 of the 24 violations on the R7-blocker list. (3) **Smallest production-code change in the remaining R-chain.** ADR-0001 §Migration plan is a mechanical move (helper bodies copied verbatim, no logic edits). (4) **Independent of every other R-phase** — does not depend on or block R2, R3, R5, or R6.
- **Production change.** Yes — moves 9 function bodies between files; backend imports swap path. No logic edits. Re-export shim preserves external callers for one release.
- **Risk.** Low (mechanical move). ADR-0001 §Risks enumerates R1.1–R1.3 with mitigations.
- **Reviewer effort.** Medium — one new file (~150 LoC of moved helpers), 3 backend import-line flips, 1 new test, 1 CHANGELOG entry. Existing pipeline tests (`test_rl_pipeline.py`, `test_sb3_pipeline.py`, `test_skrl_pipeline.py`) must stay green.
...
R0 is complete (the gate is now open). R1 / R2 / R5 / R6 fan out in
parallel. R3 must land before R4 (smaller CLI seam). R7 is the closer.
